...
def check_item_category(session: Session) -> list[Violation]:
    ...
    out: list[Violation] = []
    cat_brand = dict(session.query(Categories.name, Categories.brand_id).all())
...
def check_default_store(session: Session) -> list[Violation]:
    ...
    for b in session.query(Brand).filter(Brand.is_active.is_(True)).all():
        defaults = session.query(Store).filter(Store.brand_id == b.id, Store.is_default.is_(True)).count()
        total = session.query(Store).filter(Store.brand_id == b.id).count()
        if total == 0:
            out.append(Violation("brand_no_store", Severity.WARNING, f"brand={b.slug!r}", "active brand has no stores"))
        elif defaults == 0:
            out.append(
                Violation("brand_no_default_store", Severity.WARNING, f"brand={b.slug!r}", "no store marked is_default")
            )
        elif defaults > 1:
            out.append(
                Violation(
                    "brand_multiple_default_stores",
                    Severity.ERROR,
                    f"brand={b.slug!r}",
                    f"{defaults} stores marked is_default (must be 1)",
                )
            )
    return out
...
def check_bot_config(session: Session) -> list[Violation]:
    ...
    for b in session.query(Brand).filter(Brand.is_active.is_(True)).all():
        if b.id not in configured:
            out.append(
                Violation("brand_no_bot_config", Severity.WARNING, f"brand={b.slug!r}", "active brand has no BotConfig")
            )
    return out
...
def check_staff_store_brand(session: Session) -> list[Violation]:
    ...
    out: list[Violation] = []
    store_brand = dict(session.query(Store.id, Store.brand_id).all())
        ...
        if s.store_id not in store_brand:
            out.append(
                Violation(
                    ...
                    Severity.ERROR,
                    f"brand_staff={s.id}",
                    f"store_id={s.store_id} does not exist",
                )
            )
        elif store_brand[s.store_id] != s.brand_id:
            out.append(
                Violation(
                    "staff_store_cross_brand",
                    Severity.ERROR,
                    f"brand_staff={s.id}",
...
def check_item_sanity(session: Session) -> list[Violation]:
    ...
    for g in session.query(Goods).all():
        if g.price is None or g.price <= 0:
            out.append(Violation("item_bad_price", Severity.ERROR, f"item={g.name!r}", f"price={g.price} must be > 0"))
...
def check_item_required_fields(session: Session) -> list[Violation]:
    ...
    for g in session.query(Goods).all():
        if not g.description or not str(g.description).strip():
            out.append(
                Violation("item_missing_description", Severity.ERROR, f"item={g.name!r}", "description is empty")
            )
        if g.category_name is None or not str(g.category_name).strip():
            out.append(Violation("item_missing_category", Severity.ERROR, f"item={g.name!r}", "category_name is empty"))
...
def check_modifier_schema(session: Session) -> list[Violation]:
    ...
    for g in session.query(Goods).all():
        ...
        if not isinstance(mods, dict):
            out.append(
                Violation("modifier_bad_shape", Severity.ERROR, f"item={g.name!r}", "modifiers must be a JSON object")
            )
            continue
        for group_key, group in mods.items():
            ...
            if not isinstance(options, list) or not options:
                out.append(
                    Violation("modifier_no_options", Severity.ERROR, where, "must have a non-empty 'options' list")
                )
                continue
            seen_ids = set()
            for opt in options:
                if not isinstance(opt, dict) or "id" not in opt or "label" not in opt:
                    out.append(
                        Violation("modifier_bad_option", Severity.ERROR, where, f"option missing id/label: {opt!r}")
                    )
                    continue
                if not isinstance(opt.get("price", 0), (int, float)):
                    out.append(
                        Violation(
                            "modifier_bad_option_price",
                            Severity.ERROR,
                            where,
                ...
                if opt["id"] in seen_ids:
                    out.append(
                        Violation(
                            "modifier_duplicate_option", Severity.ERROR, where, f"duplicate option id {opt['id']!r}"
                        )
                    )
                seen_ids.add(opt["id"])
    return out
...
def check_empty_categories(session: Session) -> list[Violation]:
    ...
    for c in session.query(Categories).all():
        if c.name not in used:
            out.append(Violation("empty_category", Severity.WARNING, f"category={c.name!r}", "category has no items"))
    return out
