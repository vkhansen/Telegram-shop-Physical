...
async def query_categories(
    offset: int = 0, limit: int = 10, count_only: bool = False, brand_id: int | None = None
) -> Any:
    ...
    with Database().session() as s:
        ...
        return [
            row[0]
            for row in query.order_by(Categories.sort_order.asc(), Categories.name.asc())
            .offset(offset)
            .limit(limit)
            .all()
        ]
...
async def query_items_in_category(
    category_name: str, offset: int = 0, limit: int = 10, count_only: bool = False, brand_id: int | None = None
) -> Any:
    ...
    with Database().session() as s:
        ...
        return [row[0] for row in query.order_by(Goods.name.asc()).offset(offset).limit(limit).all()]
...
async def query_user_bought_items(user_id: int, offset: int = 0, limit: int = 10, count_only: bool = False) -> Any:
    ...
    with Database().session() as s:
        ...
        results = query.order_by(desc(BoughtGoods.bought_datetime)).offset(offset).limit(limit).all()
        s.expunge_all()
        return results
...
async def query_all_users(offset: int = 0, limit: int = 10, count_only: bool = False) -> Any:
    ...
    with Database().session() as s:
        ...
        return [
            row[0]
            for row in s.query(User.telegram_id).order_by(User.telegram_id.asc()).offset(offset).limit(limit).all()
        ]
...
async def query_admins(offset: int = 0, limit: int = 10, count_only: bool = False) -> Any:
    ...
    with Database().session() as s:
        ...
        return [row[0] for row in query.order_by(User.telegram_id.asc()).offset(offset).limit(limit).all()]
...
async def query_user_referrals(user_id: int, offset: int = 0, limit: int = 10, count_only: bool = False) -> Any:
    ...
    with Database().session() as s:
        ...
        earnings_sub = (
            s.query(
                ReferralEarnings.referral_id, func.coalesce(func.sum(ReferralEarnings.amount), 0).label("total_earned")
            )
...
async def query_referral_earnings_from_user(
    referrer_id: int, referral_id: int, offset: int = 0, limit: int = 10, count_only: bool = False
) -> Any:
    """Query earnings from specific referral"""
    with Database().session() as s:
        query = s.query(ReferralEarnings).filter(
            ReferralEarnings.referrer_id == referrer_id, ReferralEarnings.referral_id == referral_id
        )
        ...
        results = query.order_by(desc(ReferralEarnings.created_at)).offset(offset).limit(limit).all()
        s.expunge_all()
        return results


async def query_all_referral_earnings(
    referrer_id: int, offset: int = 0, limit: int = 10, count_only: bool = False
) -> Any:
    """Query all referral earnings for user"""
    with Database().session() as s:
        query = s.query(ReferralEarnings).filter(ReferralEarnings.referrer_id == referrer_id)
