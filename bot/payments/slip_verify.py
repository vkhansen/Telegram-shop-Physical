...
class SlipVerifyResult:
    status: VerifyStatus
    provider: SlipProvider | None = None
...
async def _verify_slipok(
    ...
    try:
        async with (
            ...
            session.post(url, data=form, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp,
        ):
            ...
            return SlipVerifyResult(
                ...
                amount=Decimal(str(slip_data.get("amount"))) if slip_data.get("amount") is not None else None,
                sender_name=sender_name_obj.get("en") or sender_name_obj.get("th"),
                sender_bank=(sender.get("bank") or {}).get("short"),
                receiver_name=receiver_name_obj.get("en") or receiver_name_obj.get("th"),
                receiver_bank=(receiver.get("bank") or {}).get("short"),
        ...
        async with (
            ...
            if amount_matched is False:
                ...
                result.error_message = (
                    f"Amount mismatch: slip shows {amount_in_slip} THB, expected {slip_data.get('amountInOrder')} THB"
                )
...
async def _verify_rdcw(
    ...
    try:
        async with (
            ...
            session.post(url, data=form, timeout=aiohttp.ClientTimeout(total=30)) as resp,
        ):
            data = await resp.json()
...
def _receiver_matches(expected: str, actual: str) -> bool:
    ...
    return bool(len(smaller) >= 2 and smaller <= larger)
    ...
    for provider, key1, key2 in configured:
        ...
        if result.status == VerifyStatus.VERIFIED:
            ...
            if (
                ...
                and abs(result.amount - expected_amount) > Decimal("0.01")
            ):
                logger.warning(
                ...
                result.status = VerifyStatus.AMOUNT_MISMATCH
                result.error_message = (
                    f"Amount mismatch: slip shows {result.amount} THB, expected {expected_amount} THB"
                )
                return result
            ...
            if (
                expected_receiver
                and result.receiver_name
                and not _receiver_matches(expected_receiver, result.receiver_name)
            ):
                logger.warning(
                ...
                result.error_message = (
                    f"Receiver mismatch: slip shows '{result.receiver_name}', expected '{expected_receiver}'"
                )
                return result
