...
async def receive_modifiers_json(message: Message, state):
    ...
    for group_key, group in modifiers.items():
        if not isinstance(group, dict):
            await message.answer(
                localize("admin.goods.modifiers.invalid_json", error=f"Group '{group_key}' must be an object"),
                reply_markup=back("goods_management"),
            )
            return
        if "options" not in group or not isinstance(group.get("options"), list):
            await message.answer(
                localize("admin.goods.modifiers.invalid_json", error=f"Group '{group_key}' must have 'options' array"),
                reply_markup=back("goods_management"),
            )
            return
...
async def _create_and_finalize_item(message: Message, state, admin_id: int):
    ...
    await message.answer(
        localize("admin.goods.add.result.created_with_stock", name=item_name, stock=stock_quantity) + modifier_note,
        reply_markup=back("goods_management"),
    )
