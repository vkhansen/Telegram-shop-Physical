...
class FindDealsAction(BaseModel):
    ...
    min_order_max: Decimal | None = Field(
        default=None, description="Only show coupons requiring at most this minimum order amount"
    )
...
class CheckCouponAction(BaseModel):
    ...
    order_total: Decimal | None = Field(default=None, description="Provide to show the effective discount amount")
...
class GetOrderStatusAction(BaseModel):
    ...
    order_code: str | None = Field(default=None, max_length=10)
...
class OpenSupportTicketAction(BaseModel):
    ...
    order_code: str | None = Field(default=None, max_length=10, description="Link to a specific order if relevant")
...
class StartStoreLiveChatAction(BaseModel):
    ...
    reason: str = Field(..., min_length=5, max_length=500)
    order_code: str | None = Field(default=None, max_length=10, description="Attach to a specific order if relevant")
