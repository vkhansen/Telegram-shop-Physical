            ...
            result.append(
                {
                    ...
                    "items": [{"name": it.item_name, "price": str(it.price), "qty": it.quantity} for it in items],
                }
            )
...
def _search_deliveries(action) -> dict:
    with Database().session() as s:
        q = s.query(Order).filter(Order.order_status.in_(["out_for_delivery", "delivered"]))
...
def _view_inventory(action) -> dict:
    with Database().session() as s:
        ...
        if action.only_low_stock:
            q = q.filter((Goods.stock_quantity - Goods.reserved_quantity) <= action.low_stock_threshold)
...
def _get_stats(action) -> dict:
    ...
    if action.include_top_items:
        with Database().session() as s:
            ...
            top = (
                s.query(OrderItem.item_name, func.sum(OrderItem.quantity).label("total_qty"))
                .group_by(OrderItem.item_name)
                .order_by(func.sum(OrderItem.quantity).desc())
                .limit(10)
                .all()
            )
            stats["top_items"] = [{"name": name, "total_sold": int(qty)} for name, qty in top]
...
def _lookup_user(action) -> dict:
    if action.telegram_id:
        ...
        with Database().session() as s:
            ci = s.query(CustomerInfo).filter(CustomerInfo.phone_number == action.phone_number).first()
            if ci:
...
def _exec_create_item(action, admin_id: int) -> dict:
    ...
    if action.stock_quantity > 0:
        add_inventory(action.item_name, action.stock_quantity, admin_id, comment="AI assistant create")
...
def _exec_update_item_image(action, admin_id: int) -> dict:
    ...
    media_id = _add_media_entry(
        action.item_name,
        action.photo_file_id,
        is_ai_generated=False,
    )

    logger.info("AI admin %s added image for '%s' (media_id=%s)", admin_id, action.item_name, media_id)
...
def _exec_remove_item_media(action, admin_id: int) -> dict:
    ...
    if result.get("success"):
        logger.info("AI admin %s removed media '%s' from '%s'", admin_id, action.media_id, action.item_name)
    return result
...
def _exec_bulk_price_update(action, admin_id: int) -> dict:
    ...
    for upd in action.updates:
        ...
        success, error = update_item(
            item_name, item_name, existing["description"], float(new_price), existing["category_name"]
        )
        if success:
...
def _exec_adjust_stock(action, admin_id: int) -> dict:
    ...
    if action.operation == "set":
        ...
        add_inventory(action.item_name, action.quantity, admin_id, comment=action.comment or "AI assistant add stock")
    ...
    for row in action.items:
        ...
        try:
            ...
            if existing and action.overwrite_existing:
                update_item(row.item_name, row.item_name, row.description, float(row.price), row.category_name)
                ...
                if row.stock_quantity > 0:
                    add_inventory(row.item_name, row.stock_quantity, admin_id, comment="AI bulk import")
                created.append(row.item_name)
            else:
                skipped.append(row.item_name)
...
def _exec_change_order_status(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s changed order %s: %s → %s", admin_id, action.order_code, current, action.new_status)
    return {
        "success": True,
...
def _exec_assign_driver(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s assigned driver %d to order %s", admin_id, action.driver_id, action.order_code)
...
def _exec_ban_user(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s banned user %s, reason: %s", admin_id, action.telegram_id, action.reason)
...
def _exec_toggle_coupon(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s toggled coupon '%s' to %s", admin_id, action.code.upper(), action.is_active)
...
def _exec_create_refcode(action, admin_id: int) -> dict:
    ...
    if action.expires_in_hours > 0:
        expires_at = datetime.datetime.now(datetime.UTC) + datetime.timedelta(hours=action.expires_in_hours)
...
async def _exec_send_broadcast(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s broadcast to %d users (segment: %s)", admin_id, len(user_ids), action.segment)
...
def _get_segment_users(segment: str) -> list[int]:
    ...
    with Database().session() as s:
        ...
        if segment == "high_spenders":
            ...
            users = s.query(CustomerInfo.telegram_id).filter(CustomerInfo.total_spendings >= threshold).all()
            return [u[0] for u in users]

        if segment == "recent_buyers":
            cutoff = now - datetime.timedelta(days=7)
            users = s.query(Order.buyer_id).filter(Order.created_at >= cutoff).distinct().all()
            return [u[0] for u in users if u[0]]

        if segment == "inactive":
            cutoff = now - datetime.timedelta(days=30)
            active = {u[0] for u in s.query(Order.buyer_id).filter(Order.created_at >= cutoff).distinct().all() if u[0]}
            all_users = {u[0] for u in s.query(User.telegram_id).all()}
            return list(all_users - active)

        if segment == "new_users":
            cutoff = now - datetime.timedelta(days=7)
            users = s.query(User.telegram_id).filter(User.registration_date >= cutoff).all()
            return [u[0] for u in users]
...
def _exec_toggle_store(action, admin_id: int) -> dict:
    ...
    logger.info("AI admin %s toggled store '%s' to %s", admin_id, action.store_name, action.is_active)
...
def _revenue_report(action) -> dict:
    ...
    with Database().session() as s:
        ...
        if action.include_by_payment:
            ...
            result["by_payment_method"] = {k: f"{v:.2f}" for k, v in by_payment.items()}
        ...
        if action.include_top_products:
            ...
            result["top_products"] = [
                {"name": name, "qty_sold": int(qty), "revenue": f"{float(rev):.2f}"} for name, qty, rev in items
            ]
