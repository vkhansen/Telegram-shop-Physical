...
class TestCreateOrderItems:
    ...
    def _cart(self):
        return [
            {"item_name": "Widget", "quantity": 2, "price": "50.00", "total": "100.00", "selected_modifiers": None},
            {
                "item_name": "Gadget",
                "quantity": 1,
                "price": "75.00",
    ...
    def test_returns_correct_summary_count(self, db_session, test_order):
        ...
        with patch("bot.utils.order_helpers.EnvKeys") as mock_env:
            mock_env.PAY_CURRENCY = "THB"
            summary, _to_reserve = create_order_items(db_session, test_order.id, self._cart())
