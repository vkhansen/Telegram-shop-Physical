...
from bot.database.models.main import Order, CryptoAddress, CryptoPayment
...
class TestAddressReclaim:
    ...
    def test_expired_order_releases_address(self, db_session, test_user):
        ...
        db_session.flush()
        held = CryptoAddress(
            coin="btc", address="bc1qreclaimtest", is_used=True,
            used_by=test_user.telegram_id, order_id=expired.id, used_at=stale,
        )
        db_session.add(held)
        ...
        db_session.flush()
        kept = CryptoAddress(
            coin="btc", address="bc1qactivekeep", is_used=True,
            used_by=test_user.telegram_id, order_id=active.id, used_at=stale,
        )
        db_session.add(kept)
        db_session.commit()
    ...
    def test_not_reclaimed_within_ttl(self, db_session, test_user):
        ...
        db_session.flush()
        addr = CryptoAddress(
            coin="btc", address="bc1qtooyoung", is_used=True,
            used_by=test_user.telegram_id, order_id=order.id, used_at=recent,
        )
        db_session.add(addr)
        db_session.commit()
    ...
    async def test_overpayment_recorded(self, db_session, test_user):
        ...
        verifier = AsyncMock()
        verifier.check_payment = AsyncMock(return_value=TxResult(
            found=True, tx_hash="overpaytx", amount=Decimal("0.00150000"),
            confirmations=2,
        ))
